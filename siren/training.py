...
def train(model, train_dataloader, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir, loss_fn,
          summary_fn, wandb, val_dataloader=None, double_precision=False, clip_grad=False, use_lbfgs=False,
          loss_schedules=None, filename=None, cfg=None):

    optim = torch.optim.Adam(lr=lr, params=model.parameters())
    if cfg.scheduler.type == "step":
        scheduler = StepLR(optim, step_size=cfg.scheduler.step_size, gamma=cfg.scheduler.gamma, verbose=True)
    elif cfg.scheduler.type == "adaptive":
        scheduler = ReduceLROnPlateau(optim, patience=cfg.scheduler.patience_adaptive, factor=cfg.scheduler.factor, verbose=True, threshold=cfg.scheduler.threshold, min_lr=cfg.scheduler.min_lr)
        ...
        for epoch in range(epochs):
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                pbar.set_description(
                    "Epoch %d, Total loss %0.6f, iteration time %0.6f" % (epoch, train_loss, time.time() - start_time))
                if not total_steps % steps_til_summary:
            ...
            if best_loss > epoch_loss:
                ...
                num_bad_epochs += 1
            optim.param_groups[0]["lr"] = max(optim.param_groups[0]["lr"], cfg.scheduler.min_lr)
            if cfg.strategy == "continue":
                torch.save(model.state_dict(),
                           os.path.join(checkpoints_dir, f'{filename}_model_final_{epoch}.pth'))

            wandb.log({"epoch_loss": epoch_loss, "lr": optim.param_groups[0]["lr"], "epoch": epoch})

            if num_bad_epochs == patience:
                break
        if not cfg.mlp_config.move:
            summary_fn("audio_samples", model, model_input, gt, model_output, wandb, total_steps)
