...
def create_mesh_v2(
        decoder, filename, N=256, max_batch=64 ** 3, level=0, offset=None, scale=None,
        vis_transform=None
):
    start = time.time()
    ...
    samples[:, 2] = torch.div(torch.div(overall_index, N, rounding_mode='floor'), N, rounding_mode='floor') % N

    # transform first 3 columns
    # to be the x, y, z coordinate
    mc_grid_to_world = torch.inverse(world_to_mc_grid)
    samples[:, :3] = torch.matmul(mc_grid_to_world[:3, :3], samples[:, :3].transpose(1, 0)).transpose(1,
                                                                                                      0) + mc_grid_to_world[
                                                                                                           :3, 3]
    ...
    print('samples[:,:3]', samples[:, :3].shape, torch.min(samples[:, :3]).item(), torch.max(samples[:, :3]).item())
    ...
    print('sdf_values', sdf_values.shape, torch.min(sdf_values).item(), torch.max(sdf_values).item(),
    ...
    print('sdf_tensor', sdf_tensor.shape, torch.min(sdf_tensor).item(), torch.max(sdf_tensor).item(),
          torch.mean(sdf_tensor).item())
    trunc = torch.max(sdf_tensor).item() - 0.1
    v, f = mc.marching_cubes(sdf_tensor, None, transform=torch.inverse(world_to_mc_grid), isovalue=level, truncation=trunc,
                      thresh=10 * trunc, output_filename=None if ply_filename is None else (ply_filename + ".ply"))
    ...
    for i in range(num_hist):
        hist[i] = torch.sum(
            torch.logical_and(sdf_values > hist_min + i * inc, sdf_values < hist_min + (i + 1) * inc)).item()
    splitter = ','
    with open(ply_filename + "_hist.csv", 'w') as ofs:
        ofs.write(splitter.join(['range min', 'range max', 'count']) + '\n')
        for i in range(num_hist):
            ofs.write(splitter.join([str(x) for x in [hist_min + i * inc, hist_min + (i + 1) * inc, hist[i]]]) + '\n')

def create_mesh(
    decoder, filename=None, N=256, max_batch=64 ** 3, offset=None, scale=None, level=0, time_val=-1
):
    start = time.time()
    ply_filename = filename
    ...
    while head < num_samples:
        ...
        sample_subset = samples[head : min(head + max_batch, num_samples), 0:3].cuda()
        if time_val >= 0:
            sample_subset = torch.hstack((sample_subset, torch.ones((sample_subset.shape[0], 1)).cuda() * time_val))
        samples[head : min(head + max_batch, num_samples), 3] = (
            decoder(sample_subset)
            .squeeze()#.squeeze(1)
            .detach()
