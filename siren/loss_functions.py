...
def occ_sigmoid(model_output, gt, model, cfg=None, first_state_dict=None):
    ...
    if cfg.kl_weight > 0 and first_state_dict is not None:
        ...
        curr_dist = torch.distributions.Normal(torch.mean(param_vec), torch.var(param_vec))
        param_arr = []
        for l in first_state_dict:
            param_arr.append(first_state_dict[l].flatten())
        param_vec = torch.hstack(param_arr)
        target_dist = torch.distributions.Normal(torch.mean(param_vec), torch.var(param_vec))
        kl_loss = torch.distributions.kl_divergence(curr_dist, target_dist)
        return {'occupancy': F.binary_cross_entropy(pred_sdf, gt_sdf), 'kl_weight': cfg.kl_weight * kl_loss}
    else:
        loss = F.binary_cross_entropy_with_logits(pred_sdf.squeeze(-1), gt_sdf.squeeze(-1), reduction="none")
