...
class PointCloud(Dataset):
    def __init__(self, path, on_surface_points, keep_aspect_ratio=True, is_mesh=True, output_type="occ", out_act="sigmoid", n_points=200000, cfg=None):
        super().__init__()
        ...
        if is_mesh:
            if cfg.strategy == "save_pc":
                ...
                inside_surface_values = igl.fast_winding_number_for_meshes(obj.vertices, obj.faces, points)
                thresh = 0.5
                occupancies_winding = np.piecewise(inside_surface_values,
        ...
        if not cfg.in_out:
            ...
            pc_folder = os.path.dirname(path) + "_" + str(cfg.n_points) + "_pc_" + f"{self.output_type}_in_out_{str(cfg.in_out)}"
        ...
        if self.move:
            ...
            for vert_data in vert_datas:
                ...
                uniform_coords = np.random.uniform(-0.5, 0.5, size=(n_points_uniform, 3))
                coords = np.vstack((coords, uniform_coords))
                inside_surface_values = igl.fast_winding_number_for_meshes(obj.vertices, obj.faces,
                                                                           coords)
                thresh = 0.5
                occupancies = np.piecewise(inside_surface_values,
                                                     [inside_surface_values < thresh, inside_surface_values >= thresh],
                                                     [0, 1])
                print("Inside points", (occupancies > 0.5).sum(), "All points", occupancies.shape)
                ...
                print('obj.vertices.min(), obj.vertices.max()', obj.vertices.min(), obj.vertices.max())
            ...
            point_cloud = np.load(os.path.join(pc_folder, os.path.basename(path) + ".npy"))
            self.coords = point_cloud[:, :3]
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.mlp_config.move:
            ...
            # print(coords.shape)
            return {'coords': torch.from_numpy(coords).float()}, {'sdf': torch.from_numpy(occs)}
        ...
        point_cloud_size = self.coords.shape[0] if not self.move else self.coords[time].shape[0]
        off_surface_samples = self.on_surface_points
        ...
        on_surface_normals = (self.normals[time] if self.move else self.normals)[rand_idcs, :]

        off_surface_coords = np.random.uniform(-1, 1, size=(off_surface_samples, 3))
        off_surface_normals = np.ones((len(off_surface_coords), 3)) * -1

        total_samples = self.on_surface_points + len(off_surface_normals)# + len(in_surface_normals)

        sdf = np.zeros((total_samples, 1)) if self.output_type == "sdf" else np.ones((total_samples, 1))  # on-surface = sdf:0 occ:1
        sdf[self.on_surface_points:, :] = -1 if self.output_type == "sdf" or self.out_act == "tanh" else 0  # off-surface = sdf:-1 => occ:0
        coords = np.concatenate((on_surface_coords, off_surface_coords), axis=0) # , in_surface_coords
