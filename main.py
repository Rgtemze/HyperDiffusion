...
from dataset import WeightDataset, VoxelDataset
...
def main(cfg: DictConfig):
    ...
    wandb.init(project="hyperdiffusion", dir=config["tensorboard_log_dir"],
               settings=wandb.Settings(_disable_stats=True, _disable_meta=True), tags=[Config.get("mode")],
    ...
    train_object_names = np.genfromtxt(os.path.join(dataset_path, "train_split.lst"), dtype="str")
    ...
    if method == "hyper_3d":
        ...
        all_object_names = np.array([obj for obj in os.listdir(dataset_path) if ".lst" not in obj])
        ...
        if not os.path.exists(os.path.join(dataset_path, "train_split.lst")):
            train_idx = np.random.choice(total_size, train_size + val_size, replace=False)
            ...
            print('Generating new partition', len(train_idx), train_size, len(val_idx), val_size, len(test_idx),
                  test_size)
            ...
            assert len(train_idx) == train_size and len(val_idx) == val_size and len(test_idx) == test_size

            np.savetxt(os.path.join(dataset_path, "train_split.lst"), all_object_names[list(train_idx)], delimiter=" ",
                       fmt="%s")
            np.savetxt(os.path.join(dataset_path, "val_split.lst"), all_object_names[list(val_idx)], delimiter=" ",
                       fmt="%s")
            np.savetxt(os.path.join(dataset_path, "test_split.lst"), all_object_names[list(test_idx)], delimiter=" ",
                       fmt="%s")

        val_object_names = np.genfromtxt(os.path.join(dataset_path, "val_split.lst"), dtype="str")
        val_object_names = set([str.split(".")[0] for str in val_object_names])
        test_object_names = np.genfromtxt(os.path.join(dataset_path, "test_split.lst"), dtype="str")
        ...
        train_dt = WeightDataset(mlps_folder_train, wandb_logger, model.dims, mlp_kwargs, cfg, train_object_names)
        train_dl = DataLoader(train_dt, batch_size=Config.get("batch_size"), shuffle=True, num_workers=8,
                              pin_memory=True)
        val_dt = WeightDataset(mlps_folder_train, wandb_logger, model.dims, mlp_kwargs, cfg, val_object_names)
        test_dt = WeightDataset(mlps_folder_train, wandb_logger, model.dims, mlp_kwargs, cfg, test_object_names)
    elif method == "raw_3d":
        dataset_path = os.path.join(Config.config['dataset_dir'], Config.config['dataset'])
        train_dt = VoxelDataset(dataset_path, wandb_logger, model.dims, mlp_kwargs, cfg, train_object_names)
        train_dl = DataLoader(train_dt, batch_size=Config.get("batch_size"), shuffle=True, num_workers=2)
    ...
    val_dl = DataLoader(torch.utils.data.Subset(train_dt, [0]), batch_size=1, shuffle=False)
    test_dl = DataLoader(torch.utils.data.Subset(train_dt, [0]), batch_size=1, shuffle=False)

    print("Train dataset length: {} Val dataset length: {} Test dataset length".format(len(train_dt), len(val_dt),
                                                                                       len(test_dt)))
    input_data = next(iter(train_dl))[0]
    print("Input data shape, min, max:", input_data.shape, input_data.min(), input_data.max())
    ...
    trainer = pl.Trainer(accelerator='gpu', devices=torch.cuda.device_count(), max_epochs=Config.get("epochs"),
                         strategy="ddp",
                         logger=wandb_logger,
                         default_root_dir=checkpoint_path,
                         callbacks=[best_acc_checkpoint, best_mmd_checkpoint, last_model_saver, lr_monitor],
