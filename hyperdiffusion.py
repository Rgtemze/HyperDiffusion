...
from evaluation_metrics_3d import compute_all_metrics_4d, compute_all_metrics
from hd_utils import Config, generate_mlp_from_weights, render_mesh, render_meshes, calculate_fid_3d
...
class HyperDiffusion(pl.LightningModule):
    def __init__(self, model, train_dt, val_dt, test_dt, mlp_kwargs, image_shape, method, cfg):
        super().__init__()
        self.model = model
        self.cfg = cfg
        self.method = method
        ...
        self.diff = GaussianDiffusion(betas=betas,
                                      ...
                                      loss_type=LossType[cfg.diff_config.params.loss_type], diff_pl_module=self)

    def forward(self, images):
        t = torch.randint(0, high=self.diff.num_timesteps, size=(images.shape[0],)).long().to(self.device)
    ...
    def configure_optimizers(self):
        ...
        if self.cfg.scheduler:
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=self.cfg.scheduler_step, gamma=0.9)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if "hyper" in self.method and self.trainer.global_step == 0:
            ...
            sdf_decoder = SDFDecoder(self.mlp_kwargs.model_type, None,
                                     "nerf" if self.mlp_kwargs.model_type == "nerf" else "mlp", self.mlp_kwargs)
            sdf_decoder.model = mlp.cuda()
            if not self.mlp_kwargs.move:
                sdf_meshing.create_mesh(sdf_decoder, "meshes/first_mesh", N=128,
            ...
            if self.cfg.mlp_config.params.move:
                ...
                for voxel in input_data[0]:
                    ...
                    vox_grid_mesh.vertices = rot_matrix.apply(vox_grid_mesh.vertices) * 0.9
                    img, _ = render_mesh(vox_grid_mesh)
                    out_imgs.append(img)
                self.logger.experiment.log({f"first_voxel_move": wandb.Video(
    ...
    def validation_step(self, val_batch, batch_idx):
        metric_fn = self.calc_metrics_4d if self.cfg.mlp_config.params.move else self.calc_metrics
    ...
    def training_epoch_end(self, outputs: EPOCH_OUTPUT) -> None:
        ...
        if self.method == "hyper_3d":
            if self.current_epoch % 10 == 0:
                x_0s = self.diff.ddim_sample_loop(self.model, (4, *self.image_size[1:])).cpu().float()
                x_0s = x_0s / self.cfg.normalization_factor
                print("x_0s[0].stats", x_0s.min().item(), x_0s.max().item(), x_0s.mean().item(), x_0s.std().item())
                if self.mlp_kwargs.move:
                    for i, x_0 in enumerate(x_0s):
                        out_imgs = []
                        mesh_frames, _ = self.generate_meshes(x_0.unsqueeze(0), None, res=256)
                        for mesh in mesh_frames:
                            ...
                            out_imgs.append(img)
                        self.logger.experiment.log({f"generated_renders_{i}": wandb.Video(
                            np.transpose(np.array(out_imgs), axes=(0, 3, 1, 2)), fps=12)})
                else:
                    meshes, sdfs = self.generate_meshes(x_0s, None, res=512)
                    for mesh in meshes:
                        mesh.vertices *= 2
                    print("sdfs.stats", sdfs.min().item(), sdfs.max().item(), sdfs.mean().item(), sdfs.std().item())
        ...
        # Handle Voxel baseline sample generation
            ...
            if self.current_epoch % 5 == 0:
                x_0s = self.diff.ddim_sample_loop(self.model, (self.cfg.batch_size, *self.image_size[1:])).cpu().float()
                print("x_0s[0].stats", x_0s.min().item(), x_0s.max().item(), x_0s.mean().item(), x_0s.std().item())
                if self.cfg.mlp_config.params.move:
                    rot_matrix = Rotation.from_euler("zyx", [45, 180, 90], degrees=True)
                    for i, voxel_frames in enumerate(x_0s):
                        ...
                        for voxel in voxel_frames[0]:
                            ...
                            img, _ = render_mesh(vox_grid_mesh)
                            out_imgs.append(img)
                        self.logger.experiment.log({f"generated_render_videos_{i}": wandb.Video(
                            np.transpose(np.array(out_imgs), axes=(0, 3, 1, 2)), fps=12)})
                else:
                    imgs = []
                    for vox_grid in x_0s:
                        ...
                        img, _ = render_mesh(vox_grid_mesh)
                        imgs.append(img)
                    self.logger.log_image("generated_renders", imgs, step=self.current_epoch)
    ...
    def generate_meshes(self, x_0s, folder_name="meshes", info="0", res=64, level=0):
        ...
        for i, weights in enumerate(x_0s):
            ...
            with torch.no_grad():
                effective_file_name = f"{folder_name}/mesh_epoch_{self.current_epoch}_{i}_{info}" \
                    if folder_name is not None else None
                if self.mlp_kwargs.move:
                    for i in range(16):
                        v, f, sdf = sdf_meshing.create_mesh(sdf_decoder, effective_file_name,
                                                            N=res, level=0 if self.mlp_kwargs.output_type in ["occ",
                                                                                                              "logits"] else 0,
                                                            time_val=i)  # 0.9
                        if "occ" in self.mlp_kwargs.output_type or "logits" in self.mlp_kwargs.output_type:
                    ...
                    v, f, sdf = sdf_meshing.create_mesh(sdf_decoder, effective_file_name,
                                                        N=res, level=level if self.mlp_kwargs.output_type in ["occ",
                                                                                                              "logits"] else 0)
                    if "occ" in self.mlp_kwargs.output_type or "logits" in self.mlp_kwargs.output_type:
                        tmp = copy.deepcopy(f[:, 1])
                        f[:, 1] = f[:, 2]
    ...
    def calc_metrics_4d(self, split_type):
        ...
        test_object_names = np.genfromtxt(os.path.join(dataset_path, f"{split_type}_split.lst"), dtype="str")
        ...
        sample_x_0s = []
        test_batch_size = 100 if self.cfg.method == "hyper_3d" else self.cfg.batch_size + 1
        ...
        if num_mesh_to_generates % test_batch_size != 0:
            sample_x_0s.append(
                self.diff.ddim_sample_loop(self.model, (num_mesh_to_generates % test_batch_size, *self.image_size[1:])))
        ...
        metrics = compute_all_metrics_4d(torch.tensor(sample_pcs).float().to(self.device),
                                         torch.tensor(ref_pcs).float().to(self.device), 160, self.logger)
        return metrics

    def calc_metrics(self, split_type):
        dataset_path = os.path.join(Config.config['dataset_dir'],
        ...
        if self.cfg.dedup:
            sample_dist = torch.cdist(sample_x_0s, sample_x_0s, p=2, compute_mode="donot_use_mm_for_euclid_dist")
            sample_dist_min = sample_dist.kthvalue(k=2, dim=1)[0]
            sample_dist_min_sorted = torch.argsort(sample_dist_min, descending=True)[:int(len(ref_pcs) * 1.01)]
        ...
        for x_0s in tqdm(sample_x_0s):
            if "hyper" in self.cfg.method:
                mesh, _ = self.generate_meshes(x_0s.unsqueeze(0) / self.cfg.normalization_factor, None,
        ...
        self.logger.experiment.log({'3d_gen': wandb.Object3D(r.apply(np.array(sample_pcs[0])))})
        print("Starting metric computation for", split_type)

        fid = calculate_fid_3d(sample_pcs.to(self.device), ref_pcs.to(self.device), self.logger)
        metrics = compute_all_metrics(sample_pcs.to(self.device), ref_pcs.to(self.device),
                                      16 if split_type == "test" else 16, self.logger)
    ...
    def test_step(self, *args, **kwargs):
        ...
        if self.method == "raw_3d":
            x_0s = self.diff.ddim_sample_loop(self.model, (32, *self.image_size[1:])).cpu().float()
            print("x_0s[0].stats", x_0s.min().item(), x_0s.max().item(), x_0s.mean().item(), x_0s.std().item())
            ...
            if self.cfg.mlp_config.params.move:
                ...
                for i, voxel_frames in enumerate(x_0s):
                    ...
                    for voxel in voxel_frames:
                        ...
                        vox_grid_mesh = self.grid_to_mesh(grid)
                        vox_grid_mesh.vertices = rot_matrix.apply(vox_grid_mesh.vertices) * 0.9
                        vox_grid_mesh.export(f"gen_meshes/{wandb.run.name}/voxel_animation_{i}_{len(out_imgs)}.obj")
                        img, _ = render_mesh(vox_grid_mesh)
                        out_imgs.append(img)
                    self.logger.experiment.log({f"generated_render_videos_{i}": wandb.Video(
                        np.transpose(np.array(out_imgs), axes=(0, 3, 1, 2)), fps=12)})
        ...
        # If it's HyperDiffusion, let's calculate some statistics on training dataset
            ...
            x_0s = self.diff.ddim_sample_loop(self.model, (16, *self.image_size[1:]), clip_denoised=False)
            x_0s = x_0s / self.cfg.normalization_factor
            ...
            if self.cfg.mlp_config.params.move:
                ...
                for sample_i, x_0 in enumerate(tqdm(x_0s)):
                    out_imgs = []
                    mesh_frames, _ = self.generate_meshes(x_0.unsqueeze(0), None, res=180)
                    if len(mesh_frames[0].vertices) == 0: continue
                    for mesh in mesh_frames:
                        mesh.vertices = rot_matrix.apply(mesh.vertices)
                        mesh.vertices *= 2
                        mesh.export(f"gen_meshes/{wandb.run.name}/obj_{sample_i}_frame_{len(out_imgs)}.obj")
                        # trimesh.smoothing.filter_laplacian(mesh, iterations=5)
                        img, _ = render_mesh(mesh)
                        out_imgs.append(img)
                    self.logger.experiment.log(
                        {"generated_renders": wandb.Video(np.transpose(np.array(out_imgs), axes=(0, 3, 1, 2)), fps=12)})
                return
            ...
            self.logger.log_image("generated_renders", out_imgs, step=self.current_epoch)
            self.logger.log_image("generated_renders_pc", out_pc_imgs, step=self.current_epoch)
